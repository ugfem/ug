...
void PlotVolMesh (const ROT3D & r, char key)
{
  ...
  //  if (LGM_DEBUG) MyOutTextXY (20, y, buf);
  //  y += 15;
  ...
  for (i = 1; i <= locpoints.Size(); i++)
  {
    ...
    if (testmode && i <= pindex.Size())
    {
      ...
      if (LGM_DEBUG) MyOutTextXY (r.X(locpoints[i]), r.Y(locpoints[i]), cbuf);
    }
  }
  ...
  if (testmode)
  {
    ...
    if (LGM_DEBUG) MyOutTextXY (120, 30, buf);
    if (LGM_DEBUG) MyOutTextXY (120, 45, meshing->problems[problemindex]);
  }
