...
int vwgt_max;                   /* largest vertex weight */
{
  ...
  if (largest + smallest <= vwgt_max) balanced = TRUE;
  else balanced = FALSE;
  ...
  while (!balanced) {
    /*npass++;*/
    for (i=0; i<nsets; i++) active[i] = FALSE;
    active[badset] = TRUE;
    ...
    while (!done) {
      ...
      for (i=0; i<nsets; i++) if (active[i]) dist[i] -= toobig*delta;
...
{
  extern double DOUBLE_MAX;     /* largest representable double */
  ...
  bestdelta = DOUBLE_MAX;
  while (bestdelta == DOUBLE_MAX) {
    for (i=0; i<nsets; i++) if (active[i]) {
        for (j=0; j<nsets; j++) if (!active[j]) {
            /* Look for next move from active set i to inactive set j. */
            if (toobig > 0) { from = i; to = j; }
            else { from = j; to = i; }
            minset = min(to, from);
            maxset = max(to, from);

            index = startvtx[minset][maxset];
            if (index >= nvtxs || index < 0) delta = DOUBLE_MAX;
            ...
            if (delta < bestdelta) {    /* Is this the best so far? */
              bestdelta = delta;
              bestfrom = from;
              bestto = to;
              bestdir = dir;
...
int from, to;           /* sets to be coupled */
int vtx;                /* vertex that they share */
{
  ...
  for (i=0; i<nsets; i++) coupled_sets[ncoupled][i] = 0;
...
int nsets;                      /* number of sets being divided into */
int from, to;                   /* set final vertex moved from and to */
...
double *size;                   /* sizes of the different sets */
{
  ...
  if (ncoupled == 0) return;

  if (toobig > 0) {
    done = FALSE;
    if (from == badset) done = TRUE;
    while (!done) {
      found = FALSE;
      to = from;
