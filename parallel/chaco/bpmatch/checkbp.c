...
int ndims;                      /* number of dimensions for division */
{
  ...
  for (i=1; i<=nvtxs; i++) {
    ...
    for (j=0; j<nsets; j++) {
      ...
      if (j == sets[i]) setval = val;
      if (j == 0 || val < bestval) {
        bestval = val;
        bestset = j;
      }
      if (signs[0] == 1 && signs[1] == 1) signs[2] *= -1;
      if (signs[0] == 1) signs[1] *= -1;
      signs[0] *= -1;
    }
    if (fabs(setval-bestval) >= tol*(fabs(setval)+fabs(bestval))) {
      {char buf[150]; sprintf(buf," Vtx %d in set %d (%e), but should be in %d (%e)\n", i, sets[i], setval, bestset, bestval);UserWrite(buf);}
      error = TRUE;
    }
    ++sizes[sets[i]];
  ...
  {char buf[150]; sprintf(buf," Sizes:");UserWrite(buf);}
  for (i=0; i<nsets; i++) {char buf[150]; sprintf(buf," %d(%d)", sizes[i], weights[i]);UserWrite(buf);}
  {char buf[150]; sprintf(buf,"\n");UserWrite(buf);}

  if (error) checkpnt("Error in checkbp");
}
