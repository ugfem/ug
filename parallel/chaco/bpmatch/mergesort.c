...
int *space;                     /* space for nvals integers */
{
  ...
  if (DEBUG_BPMATCH > 0) {
    flag = FALSE;
    for (i=1; i<nvals; i++) {
      if (vals[indices[i-1]] > vals[indices[i]]) flag = TRUE;
    }
    if (flag) {
      {char buf[150]; sprintf(buf,"List improperly sorted in mergesort\n");UserWrite(buf);}
      if (DEBUG_BPMATCH > 1) {
        for (i=1; i<nvals; i++) {
          {char buf[150]; sprintf(buf,"%d  %f\n", indices[i], vals[indices[i]]);UserWrite(buf);}
        }
      }
    }
  }
...
int *space;                     /* space for nvals integers */
{
  ...
  if (nvals == 1) return;
    ...
    if (vals[indices[0]] <= vals[indices[1]]) return;
    else {
      i = indices[0];
      indices[0] = indices[1];
      indices[1] = i;
...
int *space;                     /* sorted answer */
{
  ...
  /* Replace by system copy call. */
  /*   for (n1=0; n1<length1+length2; n1++) indices[n1] = space[n1];*/
  memcpy((char *)indices, (char *)space, (length1+length2)*sizeof(int));
}
