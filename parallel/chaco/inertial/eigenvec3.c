...
double *eval3;                  /* largest eigenvalue */
{
  ...
  for (i=0; i<3; i++) {
    for (j=i; j<3; j++) {
      if (fabs(H[i][j]) > xmax) xmax = fabs(H[i][j]);
    }
  }
  if (xmax != 0) {
    for (i=0; i<3; i++) {
      for (j=0; j<3; j++) mat[i][j] = H[i][j]/xmax;
    }
  }
  ...
  *eval3 = max(*eval3, root3);
  if (root1 != *eval1 && root1 != *eval3) *eval2 = root1;
  else if (root2 != *eval1 && root2 != *eval3) *eval2 = root2;
  else *eval2 = root3;
}
...
double *res;                    /* returned error estimate */
{
  ...
  for (i=0; i<3; i++) for (j=0; j<3; j++) mat[i][j] = A[i][j];
  for (i=0; i<3; i++) mat[i][i] -= eval;
