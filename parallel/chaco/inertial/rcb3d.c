...
void rcb3d(graph, nvtxs, ndims, x, y, z, sets, goal, using_vwgts, part_type)
struct vtx_data **graph;	/* graph data structure for weights */
int nvtxs;			/* number of vtxs in graph */
int ndims;			/* number of cuts to make at this step */
float *x, *y, *z;		/* x and y coordinates of vertices */
   ...
   /* calculate the bisection coordinate direction */
{
	...
	if (rcbdir == 0)
    	if ((max_x-min_x) < (max_z-min_z)) rcbdir = 2;
	else
    	if ((max_y-min_y) < (max_z-min_z)) rcbdir = 2;
}
