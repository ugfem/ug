...
void inertial2d(graph, nvtxs, ndims, x, y, sets, goal, using_vwgts, part_type)
...
int part_type;
{
   ...
   if (DEBUG_INERTIAL > 0) {
     {char buf[150]; sprintf(buf,"Principle Axis = (%g, %g), Eval=%g, Residual=%e\n", evec[0], evec[1], eval, res);UserWrite(buf);}
   }
