...
void make_subgraph(graph, subgraph, subnvtxs, psubnedges, assignment, set, glob2loc,
                   loc2glob, degree, ready)
...
short *ready;
{
  ...
  int tempwgt;                  /* weight of vertex being swapped */
  ...
  for (i=1; i<=subnvtxs; i++) {
    ...
    if (using_ewgts) fptr = subgptr->ewgts + 1;
    for (j=1; j<newnedges; ) {
      neighbor = *iptr;
      if ((assignment[neighbor] == set)&&(ready[neighbor]==FALSE)) {
    ...
    subnedges += newnedges;
    if (using_ewgts) subgptr->ewgts[0] = -ewgtsum;
  }
...
void remake_graph(subgraph, subnvtxs, loc2glob, degree)
struct vtx_data **subgraph;     /* subgraph data structure */
int subnvtxs;                   /* number of vtxs in subgraph */
int *loc2glob;                  /* mapping from subgraph to graph numbering */
short *degree;                  /* degrees of vertices in graph */
{
  ...
  /* no subgraph to remake */
  if (subnvtxs == 0) return;

  using_ewgts = (subgraph[1]->ewgts != NULL);
  ...
  for (i=1; i<=subnvtxs; i++) {
    ...
    if (using_ewgts) {
      ewgtsum = 0;
      fptr = subgptr->ewgts;
      for (j=degree[i]-1; j; j--) ewgtsum += *(++fptr);
