...
int nmyvtxs;                    /* number of vertices I own */
{
  ...
  while (fabs(step) >= step_min && (!pdflag || fabs(grad) > grad_min)) {
    ...
    if (hess < -pdtol) pdflag = FALSE;
    else pdflag = TRUE;

    hess_min = hfact*fabs(grad)/step_max;
    if (hess < hess_min) hess = hess_min;

    if (fabs(grad) > fabs(hess*step_max)) step = -step_max*sign(grad);
    else step = -grad/hess;
  ...
  if (DEBUG_OPTIMIZE > 0) {
    {char buf[150]; sprintf(buf,"After %d passes, func=%e, theta = %f\n", i, func, theta);UserWrite(buf);}
  }
