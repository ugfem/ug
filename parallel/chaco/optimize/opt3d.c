...
/* Compute rotation angle to minimize distance to discrete points. */
{
  ...
  double constraint();
  double drand48();
  void grad3d(), hess3d(), gradcon(), hesscon(), kramer3(), eigenvec3(), evals3();
  ...
  hess_tol = 1.0e-6;
  pdtol = 1.0e-7;
  max_constraint = 1.0e-14*a;
  mfactor = 20.0;
  mstart = 5.0*a;

  for (i=0; i<25; i++) coeffs[i] = 0;
  ...
  /* Note that the relative sizes of funcf and funcc depend on normalization
     of eigenvectors, and I'm assuming them normalized to 1. */
  for (i=0; i<15; i++) coeffs[i] *= nvtxs;
  a = sqrt((double) nvtxs);
  for (i=15; i<25; i++) coeffs[i] *= a;

  bestf = 0;
  maxtries = OPT3D_NTRIES;
  for (ntries=1; ntries <= maxtries; ntries++) {
    /* Initialize the starting guess randomly. */
    vars[0] = TWOPI*(drand48()-.5);
    vars[1] = acos(2.0*drand48()-1.0) - HALFPI;
    vars[2] = TWOPI*(drand48()-.5);
    ...
    while (funcc >= max_constraint) {
      ...
      while (step_size >= step_min && (!pdflag || grad_norm > grad_min)) {
        ...
        for (i=0; i<3; i++) vars[i] += step[i];
        inner++;
      }
      if (inner1 == 0) inner1 = inner;
      total += inner;
      mult *= mfactor;
    }

    if (DEBUG_OPTIMIZE > 0) {
      {char buf[150]; sprintf(buf,"On try %d, After %d (%d) passes, funcf=%e, funcc=%e (%f, %f, %f)\n",
                              ntries, total, inner1, funcf, funcc, vars[0], vars[1], vars[2]);UserWrite(buf);}
    }

    if (ntries == 1 || funcf < bestf) {
      bestf = funcf;
      for (i=0; i<3; i++) best[i] = vars[i];
    }
  }
  *ptheta = best[0];
  *pphi = best[1];
