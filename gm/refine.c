...
INT CreateGridOverlap (MULTIGRID *theMG)
{
  INT l,i,prio,s;
  ...
  /*
     ...
     INT	ConnectNewOverlap (MULTIGRID *theMG)
...
IdentifyProcBoundaryObjects(theMG);

/* create one-element-overlapping for multigrid */
CreateGridOverlap(theMG);
ConnectNewOverlap(theMG);
        #endif
