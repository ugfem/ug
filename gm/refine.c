...
#	endif
		...
		theElement=SUCCE(theElement))
	{
		...
		{
			MyEdge=GetEdge(CORNER(theElement,CORNER_OF_EDGE(theElement,j,0)),
	...
	/* reset EDGE/SIDEPATTERN in elements, set SIDEPATTERN in elements */
	/* for quadrilateral sides and set PATTERN on the edges            */
	for (theElement=FIRSTELEMENT(theGrid); theElement!=NULL; 
			theElement=SUCCE(theElement))
	{
		/* TODO: delete special debug */ PRINTELEMID(11668)
		...
		if (MARKCLASS(theElement)==RED_CLASS)
		{
			...
			for (i=0; i<EDGES_OF_ELEM(theElement); i++)
				...
				{
					MyEdge=GetEdge(CORNER(theElement,
...
FIFOSTART:
		...
		theElement=SUCCE(theElement))
	{
		...
		{
			MyEdge=GetEdge(CORNER(theElement,CORNER_OF_EDGE(theElement,i,0)),
		...
		for (i=0; i<SIDES_OF_ELEM(theElement); i++)
		{
			...
			/* edgepatterns from theElement and NbElement are in final state */
			...
			switch (CORNERS_OF_SIDE(theElement,i))
			{
				case 3:
					...
					MyEdgeNum = TriSectionEdge[MyEdgePattern
									&CondensedEdgeOfSide[i]][0];

					if (MyEdgeNum == -2) RETURN(-1);
					...
					switch (TAG(NbElement))
					{
						case TETRAHEDRON:
							...
							{
								NbEdge=GetEdge(CORNER(NbElement,
												CORNER_OF_EDGE(NbElement,k,0)),
											   CORNER(NbElement,
												CORNER_OF_EDGE(NbElement,k,1)));
								ASSERT(NbEdge!=NULL);
								NbEdgePattern = NbEdgePattern | 
												(PATTERN(NbEdge)<<k);
							}
							...
							if (!( CORNER(theElement,
								   CORNER_OF_EDGE(theElement,MyEdgeNum,0)) == 
								   CORNER(NbElement,
								   CORNER_OF_EDGE(NbElement,NbEdgeNum,0)) &&
								   CORNER(theElement,
								   CORNER_OF_EDGE(theElement,MyEdgeNum,1)) == 
								   CORNER(NbElement,
								   CORNER_OF_EDGE(NbElement,NbEdgeNum,1))	) 
								&&
								!( CORNER(theElement,
								   CORNER_OF_EDGE(theElement,MyEdgeNum,0)) == 
								   CORNER(NbElement,
						...
						case PRISM: 
						{
							...
							for (k=0; k<CORNERS_OF_SIDE(NbElement,j); k++)
							{
								...
								NbEdge=GetEdge(CORNER(NbElement,
											CORNER_OF_EDGE(NbElement,edge,0)),
											CORNER(NbElement,
											CORNER_OF_EDGE(NbElement,edge,1)));
								ASSERT(NbEdge!=NULL);
							...
							if (MyEdgeNum != trisectionedge)
								SETSIDEPATTERN(NbElement,
									SIDEPATTERN(NbElement)|(1<<j)); 
		...
		theElement=SUCCE(theElement))
	{
		...
		IFDEBUG(gm,1)
		UserWriteF("    ID=%d TAG=%d MyPattern=%d Mark=%d MyEdgePattern=%d "
			"MySidePattern=%d\n",ID(theElement),TAG(theElement),MyPattern,
			Mark,MyEdgePattern,MySidePattern);
		ENDDEBUG
		...
		if (fifoFlag)
		{
          ...
          if (MARKCLASS(theElement)==RED_CLASS && MyPattern != NewPattern)
		  {
				...
				for (j=0; j<EDGES_OF_ELEM(theElement); j++)
				{
                     if (EDGE_IN_PAT(MyPattern,j)==0 && 
						 EDGE_IN_PAT(NewPattern,j))
					 {

						MyEdge=GetEdge(CORNER(theElement,
											CORNER_OF_EDGE(theElement,j,0)),
										CORNER(theElement,
											CORNER_OF_EDGE(theElement,j,1)));
						ASSERT(MyEdge != NULL);
					...
					if (EDGE_IN_PAT(MyPattern,j) && 
						EDGE_IN_PAT(NewPattern,j)==0)
					{

						UserWriteF("CloseGrid(): ERROR EID=%d in fifo "
							"MyPattern=%d has edge=%d refined but "
							"NewPattern=%d NOT!\n",
			...
			theElement=SUCCE(theElement))
	{
		...
		for (i=0; i<EDGES_OF_ELEM(theElement); i++)
		{
			...
			{
				/* for hexhedra and pyramids Patterns2Rules returns 0       */
				/* for non red elements                                     */
...
*/
			if (theNeighbor !=NULL)
			{
				...
				IFDEBUG(gm,3)
				UserWriteF("    	Searching for side node allocated by "
					"green neighbor:\n");
				ENDDEBUG
			...
			for (j=0; j<EDGES_OF_SIDE(theElement,i); j++)
			{
				...
				fatherEdge = GetEdge(CORNER(theElement,
							 		  CORNER_OF_EDGE(theElement,
									   EDGE_OF_SIDE(theElement,i,j),0)),
									 CORNER(theElement,
									  CORNER_OF_EDGE(theElement,
									   EDGE_OF_SIDE(theElement,i,j),1)));
					 ...
					 NODE *SideNodes[MAX_SIDE_NODES])
{
	...
	{
		SideNodes[i] = SONNODE(CORNER(theElement,
	...
	IFDEBUG(gm,4)
		...
		for (i=0; i<*Sons_of_Side; i++)
			UserWriteF("son[%d]=%08x/%x\n",i,
				DDD_InfoGlobalId(PARHDRE(SonList[i])),SonList[i]);
	ENDDEBUG
	#endif
					 ...
					 ELEMENT **Elements, INT *Sides)
{
	...
	for (i=0; i<nelems; i++)
	{
		...
		for (j=0; j<CORNERS_OF_SIDE(Entry->elem,Entry->side);j++)
			Entry->nodeptr[j] = CORNER(Entry->elem,
									CORNER_OF_SIDE(Entry->elem,Entry->side,j));
			...
			if ( (side = SON_NB(sdata,i)) < FATHER_SIDE_OFFSET )
			{
				...
				IFDEBUG(gm,3)
					...
					for (p=0; p<CORNERS_OF_SIDE(SonList[s],i); p++)
						UserWriteF(" %2d",ID(CORNER(SonList[s],
							CORNER_OF_SIDE(SonList[s],i,p))));
					...
					for (ss=0; ss<SIDES_OF_ELEM(SonList[side]); ss++)
					{
						...
						for (pp=0; pp<CORNERS_OF_SIDE(SonList[s],i); pp++)
							for (qq=0; qq<CORNERS_OF_SIDE(SonList[side],ss); qq++)
								if (CORNER(SonList[s],
										CORNER_OF_SIDE(SonList[s],i,pp)) == 
									CORNER(SonList[side],
										CORNER_OF_SIDE(SonList[side],ss,qq)))
								{
									pts |= ((1<<pp) | (16<<qq));
									break;
								}
					...
					for (pp=0; pp<CORNERS_OF_SIDE(SonList[side],ss); pp++)
						UserWriteF(" %2d",ID(CORNER(SonList[side],
							CORNER_OF_SIDE(SonList[side],ss,pp))));
					}
					UserWriteF("\n\n");
				ENDDEBUG
...
/*
	...
	DisposeTopLevel(theMG);
	{
		...
		dddif_SetOverlapPriorities(GRID_ON_LEVEL(theMG,TOPLEVEL(theMG)));
	}
	#endif
