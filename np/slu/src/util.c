...
static float __ftime, __fflops, __stime, __sflops;
void SLU_GetStat (float *ftime, float *fflops, float *stime, float *sflops)
{
  *ftime=__ftime;
  *fflops=__fflops;
  *stime=__stime;
...
void SetStat (SuperLUStat_t *SuperLUStat)
{
  double         *utime;
  flops_t        *ops;
  ...
  ops   = SuperLUStat->ops;
  __ftime=utime[FACT];
  if (utime[FACT] != 0.0) __fflops=ops[FACT]/utime[FACT];
  else __fflops=-1.0;
  __stime=utime[SOLVE];
  if (utime[SOLVE]!=0.0) __sflops=ops[SOLVE]/utime[SOLVE];
  else __sflops=-1.0;
