                          ...
                          VECDATA_DESC **ev, VECDATA_DESC *b, INT display)
{
  ...
  for (i=0; i<m; i++) {
    ...
    if (s_ddot(theMG,0,level,ev[i],b,scal)!=NUM_OK)
      return(1);
    scalP = 0.0;
    for (j=0; j<VD_NCOMP(b); j++)
      scalP -= scal[j];
    for (j=0; j<VD_NCOMP(b); j++)
      scal[j] = scalP;
    if (display == PCR_FULL_DISPLAY)
      UserWriteF(" %lf",scalP);
    if (a_daxpy(theMG,0,level,ev[m],EVERY_CLASS,scal,ev[i])!=NUM_OK)
      return(1);
  }
  ...
  while (i < nev) {
    ...
    for (iter=0; iter<np->maxiter; iter++) {
      ...
      if ((*np->LS->Defect)(np->LS,level,ev[i],np->r,np->M,
                            &ewresult->error_code))
        return (1);
      if ((*np->LS->Residuum)(np->LS,level,ev[i],np->r,np->M,
                              &ewresult->lresult[i]))
        return (1);
